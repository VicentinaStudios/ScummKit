...
class GenerateSCUMM: BaseCodeGenerator<ScummOpcode> {
    ...
    override func generateByteCode(statements: [Statement]) throws -> Chunk {
        ...
        try statements.forEach { statement in
            ...
            let stmt = (statement as? ExpressionStmt)
            line = (stmt?.expression as? BinaryExpression)?.operatorToken.line
                ?? (stmt?.expression as? UnaryExpression)?.operatorToken.line
    ...
    override func visitLiteralExpr(_ expression: LiteralExpression) -> Any? {
